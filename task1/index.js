...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    <h2>${hero.name}</h2>
    <div><img src="${hero.imgUrl}"></div>
    <div>Силы: ${hero.features}</div>
    <div>Подробнее: ${hero.descrip}</div>
    <label for="${hero.name}"><b>Твоя оценка</b></label>
   
    <input type="radio" id="${hero.name}" value="1"  class="rate" name="${hero.name}"/>
    <label for="st1"></label>
    <input type="radio" id="${hero.name}" value="2" class="rate" name="${hero.name}"/>
    <label for="st2"></label>
    <input type="radio" id="${hero.name}" value="3" class="rate" name="${hero.name}"/>
    <label for="st3"></label>
    <input type="radio" id="${hero.name}"  value="4" class="rate" name="${hero.name}"/>
    <label for="st4"></label>
    <input type="radio" id="${hero.name}" value="5" class="rate" name="${hero.name}"/>
    <label for="st5"></label>
    <input type="radio" id="${hero.name}" value="6"  class="rate" name="${hero.name}"/>
    <label for="st6"></label>
    <input type="radio" id="${hero.name}" value="7" class="rate" name="${hero.name}"/>
    <label for="st7"></label>
    <input type="radio" id="${hero.name}" value="8" class="rate" name="${hero.name}"/>
    <label for="st8"></label>
    <input type="radio" id="${hero.name}" value="9" class="rate" name="${hero.name}"/>
    <label for="st9"></label>
    <input type="radio" id="${hero.name}" value="10" class="rate" name="${hero.name}"/>
    <label for="st10"></label>
    </div>`;

		document.querySelector(".container").innerHTML = heroesContent;
	}
	let raz = document.querySelectorAll(".rate");
	...
	/*if (localStorage.getItem(key)) {
		...
		document
			.querySelector(
				'input[name="${hero.name}"][value="' + newValue + '"]'
			)
